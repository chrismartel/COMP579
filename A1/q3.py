...
def main():
    ...
    q = np.zeros((n_experiments, K, n_trials))
        ...
        for exp in range(n_experiments):
            ...
            for arm in range(K):
                for trial in range(n_trials):
                    bandit.N += 1
                    if (update_method == 0):
                        q[exp, arm,trial] = bandit.updateAvg(arm, rewards[arm,trial])
                    else:
                        q[exp, arm,trial] = bandit.update(arm, rewards[arm,trial], alphas[update_method-1])
        
        label = "averaging method" if update_method == 0 else "fixed learning method - alpha = {alpha}".format(alpha=alphas[update_method-1])

        for arm in range(K):
            axes[arm].plot(np.arange(n_trials), np.mean(q, axis=0)[arm], label=label)
