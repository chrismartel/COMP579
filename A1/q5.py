        ...
        for _, epsilon in enumerate(epsilons):
            ...
            for exp in range(n_experiments):
                ...
                for trial in range(n_trials):
                    ...
                    bandit.N[arm] += 1
                    bandit.update(arm, reward_received[exp, trial])
            ...
            fig.suptitle("alpha = {alpha} - epsilon = {epsilon}".format(alpha=alpha, epsilon=epsilon))
            plt.savefig("plots/q5_alpha={alpha}_epsilon={epsilon}.png".format(alpha=alpha, epsilon=epsilon))
